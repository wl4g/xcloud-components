...
public class PageWrapper<E> implements Serializable {
	...
	public PageWrapper(Integer pageNum, Integer pageSize) {
		setPageNum(pageNum);
	...
	public PageWrapper<E> setCurrentContextPage() {
		setCurrentContextPage(this);
		return this;
	}

	public static void setCurrentContextPage(PageWrapper<?> page) {
		if (nonNull(rpcContextGetHolderMethod) && nonNull(rpcContextSetMethod)) {
	...
	/**
	 * Gets {@link PageWrapper} from Rpc context.
	 * 
	 * @return
	 */
	public static final PageWrapper<?> getCurrentContextPage() {
		if (nonNull(rpcContextGetHolderMethod) && nonNull(rpcContextGetMethod)) {
			Object holder = invokeMethod(rpcContextGetHolderMethod, null);
			return (PageWrapper<?>) invokeMethod(rpcContextGetMethod, holder,
					new Object[] { CURRENT_PAGE_KEY, PageWrapper.class });
		}
		return null;
	}
	...
	// Rpc context methods.
	private static final Class<?> rpcContextHolderClass = resolveClassNameNullable(
			"com.wl4g.component.rpc.feign.context.RpcContextHolder");
	private static final Method rpcContextGetHolderMethod = findMethodNullable(rpcContextHolderClass, "get");
	private static final Method rpcContextGetMethod = findMethodNullable(rpcContextHolderClass, "get", String.class, Class.class);
	private static final Method rpcContextSetMethod = findMethodNullable(rpcContextHolderClass, "set", String.class,
			Object.class);

	// Page save to context key.
	private static final String CURRENT_PAGE_KEY = PageWrapper.class.getSimpleName().concat(".currentPage");
