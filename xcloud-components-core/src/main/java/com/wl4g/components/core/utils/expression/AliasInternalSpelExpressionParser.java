 ...
 * @since 3.0
 */
class AliasInternalSpelExpressionParser extends TemplateAwareExpressionParser {
	 ...
	 */
	public AliasInternalSpelExpressionParser(SpelParserConfiguration configuration) {
	...
	private SpelNodeImpl eatExpression() {
		...
		if (t != null) {
			...
			if (t.kind == TokenKind.ELVIS) { // a?:b (a if it isn't null,
												// otherwise b)
				if (expr == null) {
	...
	private void eatConstructorArgs(List<SpelNodeImpl> accumulatedArguments) {
		if (!peekToken(TokenKind.LPAREN)) {
			throw new InternalParseException(
					new SpelParseException(this.expressionString, positionOf(peekToken()), SpelMessage.MISSING_CONSTRUCTOR_ARGS));
		}
	...
	private boolean maybeEatMethodOrProperty(boolean nullSafeNavigation) {
		if (peekToken(TokenKind.IDENTIFIER)) {
			...
			// method reference
			push(new MethodReference(nullSafeNavigation, methodOrPropertyName.stringValue(), methodOrPropertyName.startPos,
					methodOrPropertyName.endPos, args));
			// TODO what is the end position for a method reference? the name or
			// the last arg?
			return true;
		}
		return false;
	...
	private boolean maybeEatConstructorReference() {
		if (peekIdentifierToken("new")) {
			...
			// It looks like a constructor reference but is NEW being used as a
			// map key?
			if (peekToken(TokenKind.RSQUARE)) {
			...
			if (peekToken(TokenKind.LSQUARE)) {
				...
				push(new ConstructorReference(newToken.startPos, newToken.endPos, dimensions.toArray(new SpelNodeImpl[0]),
						nodes.toArray(new SpelNodeImpl[0])));
			} else {
	...
	private boolean peekToken(TokenKind desiredTokenKind, boolean consumeIfMatched) {
		...
		if (desiredTokenKind == TokenKind.IDENTIFIER) {
			...
			// != ) -
			// in which case we can treat it as an identifier. The list is
			// represented here:
			// Tokenizer.alternativeOperatorNames and those ones are in order in
			// the TokenKind enum.
			if (t.kind.ordinal() >= TokenKind.DIV.ordinal() && t.kind.ordinal() <= TokenKind.NOT.ordinal() && t.data != null) {
				// if t.data were null, we'd know it wasn't the textual form, it
				// was the symbol form
				return true;
			}
		}
