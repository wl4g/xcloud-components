 ...
 */
package com.wl4g.component.integration.shardingsphere.sharding.algorithm;
...
/**
 * {@link SmartModShardingDatabaseAlgorithm}
 * 
...
@Getter
@Setter
public class SmartModShardingDatabaseAlgorithm implements StandardShardingAlgorithm<Comparable<?>>, ShardingAutoTableAlgorithm {
	...
	private boolean isContainAllTargets(final RangeShardingValue<Comparable<?>> shardingValue) {
		return !shardingValue.getValueRange().hasUpperBound()
				|| shardingValue.getValueRange().hasLowerBound() && getLongValue(shardingValue.getValueRange().upperEndpoint())
						- getLongValue(shardingValue.getValueRange().lowerEndpoint()) >= shardingCount - 1;
	}
