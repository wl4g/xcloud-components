...
public final class ShardingConfigurationLoader {
	 ...
	 *
	 * @param path
	 *            configuration path of Sharding-Proxy
	...
	public ShardingConfiguration load(final String path) throws IOException {
		...
		YamlProxyServerConfiguration serverConfig = loadServerConfiguration(
				new File(ShardingConfigurationLoader.class.getResource(path + "/" + SERVER_CONFIG_FILE).getFile()));
		File configPath = new File(URLDecoder.decode(ShardingConfigurationLoader.class.getResource(path).getFile(), "UTF-8"));
		...
		for (File each : findRuleConfigurationFiles(configPath)) {
			loadRuleConfiguration(each, serverConfig).ifPresent(yamlProxyRuleConfiguration -> {
				Preconditions.checkState(schemaNames.add(yamlProxyRuleConfiguration.getSchemaName()),
						"Schema name `%s` must unique at all rule configurations.", yamlProxyRuleConfiguration.getSchemaName());
		...
		Map<String, YamlProxyRuleConfiguration> ruleConfigurationMap = ruleConfigurations.stream()
				.collect(Collectors.toMap(YamlProxyRuleConfiguration::getSchemaName, each -> each));
	...
	private Optional<YamlProxyRuleConfiguration> loadRuleConfiguration(final File yamlFile,
		...
		YamlProxyRuleConfiguration result = YamlEngine.unmarshal(yamlFile, YamlProxyRuleConfiguration.class,
				new YamlProxyRuleConfigurationConstructor());
		if (null == result) {
