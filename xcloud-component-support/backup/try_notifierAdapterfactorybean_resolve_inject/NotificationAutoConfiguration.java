...
public class NotificationAutoConfiguration {
	...
	public FactoryBean<GenericOperatorAdapter<NotifierKind, MessageNotifier>> compositeMessageNotifierAdapter(
			List<MessageNotifier> notifiers) {
		return new FactoryBean<GenericOperatorAdapter<NotifierKind, MessageNotifier>>() {

			private volatile GenericOperatorAdapter<NotifierKind, MessageNotifier> notifierAdapter;

			@Override
			public GenericOperatorAdapter<NotifierKind, MessageNotifier> getObject() throws Exception {
				return obtainNotifierAdapterInstance();
			}

			@SuppressWarnings("rawtypes")
			@Override
			public Class<GenericOperatorAdapter> getObjectType() {
				return GenericOperatorAdapter.class;
			}

			/**
			 * Create singleton {@link GenericOperatorAdapter}
			 * 
			 * @return
			 */
			private final GenericOperatorAdapter<NotifierKind, MessageNotifier> obtainNotifierAdapterInstance() {
				if (isNull(notifierAdapter)) {
					synchronized (this) {
						if (isNull(notifierAdapter)) {
							this.notifierAdapter = new GenericOperatorAdapter<NotifierKind, MessageNotifier>(
